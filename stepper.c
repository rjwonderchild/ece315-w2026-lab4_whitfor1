...
stepper_mode_t step_mode = FULL_STEP_MODE;
XGpio PModMotorInst;
...
void Stepper_setNextFullStep(int direction){
    ...
    if(step_mode == FULL_STEP_MODE){
        if (stepPhase <= -1) stepPhase = 3;
        if (stepPhase >= 4) stepPhase = 0;
    ...
    else if(step_mode == WAVE_DRIVE_MODE){
        if (stepPhase <= -1) stepPhase = 3;
        if (stepPhase >= 4) stepPhase = 0;

        switch(stepPhase){
            case 0: XGpio_DiscreteWrite(&PModMotorInst, 1, WAVE_DRIVE_1); break;
            case 1: XGpio_DiscreteWrite(&PModMotorInst, 1, WAVE_DRIVE_2); break;
            case 2: XGpio_DiscreteWrite(&PModMotorInst, 1, WAVE_DRIVE_3); break;
            case 3: XGpio_DiscreteWrite(&PModMotorInst, 1, WAVE_DRIVE_4); break;
        }
    }

    else if(step_mode == HALF_STEP_MODE){
        if (stepPhase <= -1) stepPhase = 7;
        if (stepPhase >= 8) stepPhase = 0;

        switch(stepPhase){
            case 0: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_1); break;
            case 1: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_2); break;
            case 2: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_3); break;
            case 3: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_4); break;
            case 4: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_5); break;
            case 5: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_6); break;
            case 6: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_7); break;
            case 7: XGpio_DiscreteWrite(&PModMotorInst, 1, HALF_STEP_8); break;
        }
    }
}
